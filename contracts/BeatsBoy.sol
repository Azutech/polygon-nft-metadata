...
contract BeatsBoy is ERC721URIStorage {
    ...
    constructor() ERC721("BeatsBoy", "BTSB") {
    ...
    function generateCharacter(uint256 tokenId) public pure returns (string memory){
        ...
        bytes memory svg = abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350">'
                ...
                '<text x="50%" y="40%" class="base" dominant-baseline="middle" text-anchor="middle">BeatBoy</text>'
    ...
    function getTokenURI(uint256 tokenId) public pure returns (string memory){
        bytes memory dataURI = abi.encodePacked(
            '{',
            '"name": "Beats Boy #', tokenId.toString(), '",',
            '"description": "Beats on chain",',
